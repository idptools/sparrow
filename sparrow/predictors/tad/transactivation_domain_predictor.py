...
"""
NB: This network and predictor was imported from GOOSE, so is subtly different internally to how
some of the other predictors work. Notably it includes a softmax project and a loop 
this loop below to define probabilities - this may be because these networks have 2 layers 
whereas the others only have one? Anyway, just making a note of this in case we need to debug in 
the future.

        score = []
        for val in prediction:
            score.append(round(val[1],5))
...
class TADPredictor():
    ...
    Class that loads in a network such that predict_ser_phosphorylation() can be called to predict
    serine phosphorylation from a sequence.
    ...
    def __init__(self, version=None):
        ...
        saved_weights = sparrow.get_data(f'networks/transactivation_domains/tad_predictor_network_v{version}.pt')
    ...
    def predict_transactivation_domains(self, seq):
        """
        Function to predict the presence of nuclear import signals. Returns a per
        residue probability score of a residue being in an NLS or not
        ...
        np.ndarray
            ...
            gives the prediction of that residue being an NLS
