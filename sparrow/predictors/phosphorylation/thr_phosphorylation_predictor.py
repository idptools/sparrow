...
## CHANGE class name
class ThrPhosphorylationPredictor():
    """

    Class that loads in a network such that predict_thr_phosphorylation() can be called to predict
    threonine phosphorylation from a sequence.
    ...
    def __init__(self, version=None):
        ...
        saved_weights = sparrow.get_data(f'networks/phosphorylation/thr_phosphorylation_predictor_network_v{version}.pt')
    ...
    def predict_thr_phosphorylation(self, seq, raw_values=False, return_sites_only=False, windowsize=4, threshold=0.6):
        """
        Function to predict the presence of phosphothreonine residues. By default returns a 
        ...
        if raw_values:
            ...
            if return_sites_only:
                return phospho_predictor_utils.return_hits(seq, phospho_probability, 'T', windowsize=windowsize, threshold=threshold, return_sites_only=True)
            else:
                return np.array(phospho_predictor_utils.return_hits(seq, phospho_probability, 'T', windowsize=windowsize, threshold=threshold))
