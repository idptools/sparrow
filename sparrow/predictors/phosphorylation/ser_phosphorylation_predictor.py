...
## CHANGE class name
class SerPhosphorylationPredictor():
    """
    ...
    def __init__(self, version=None):
        ...
        saved_weights = sparrow.get_data(f'networks/phosphorylation/ser_phosphorylation_predictor_network_v{version}.pt')
    ...
    def predict_ser_phosphorylation(self, seq, raw_values=False, return_sites_only=False, windowsize=4, threshold=0.6):
        """
        Function to predict the presence of phosphoserine residues. By default returns a 
        ...
        if raw_values:
            return np.array(s_phospho_probability)
        
        else:
            if return_sites_only:
                return phospho_predictor_utils.return_hits(seq, s_phospho_probability, 'S', windowsize=windowsize, threshold=threshold, return_sites_only=True)
            else:
                return np.array(phospho_predictor_utils.return_hits(seq, s_phospho_probability, 'S', windowsize=windowsize, threshold=threshold))
