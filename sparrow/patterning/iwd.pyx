...
cdef double __compute_bivariate_charge_IWD(DOUBLE_t[:] linear_NCPR, INT64_t[:] negative_indices, int num_negative, INT64_t[:] positive_indices, int num_positive, str sequence):
    """
    Internal Cython function to compute the bivariate charge-weighted inverse weighted distance (IWD) 
    between negative and positive residues in the given sequence. This function calculates the IWD 
    based on the positions and charges of specified residues.

    Parameters
    ----------
    linear_NCPR : memoryview of np.ndarray[double]
        A memory view of a numpy array representing the linear net charge per residue (NCPR) values 
        for the sequence.

    negative_indices : memoryview of np.ndarray[int]
        A memory view of a numpy array containing the indices of negative residues in the sequence.

    num_negative : int
        The number of negative residues.

    positive_indices : memoryview of np.ndarray[int]
        A memory view of a numpy array containing the indices of positive residues in the sequence.

    num_positive : int
        The number of positive residues.
    ...
    -------
    double
        The computed bivariate charge-weighted inverse weighted distance (IWD) value between negative 
        and positive residues in the sequence.

    Notes
    -----
    - This function calculates the IWD based on the charges and positions of specified residues.
    - The function first filters the linear NCPR values based on the residue type (DE for negative, 
      RK for positive) and assigns the filtered values to a new array.
    - It then iterates over pairs of negative and positive residues, calculating the absolute difference 
      in charge and dividing it by the absolute distance between the residues, summing these values.
    - Finally, it returns the average of these calculated distances weighted by the number of negative 
      residues.
    """
    cdef:
        Py_ssize_t i, j
